...
async def send_message(channel_id: int):
    if os.path.exists(f".Reports/{now_date}"):
        if  not(os.listdir('./Reports/') == 0):
            doc1 = FSInputFile(path=rf'./Reports/{now_date}/Графики_по_тестам_{now_date}.pdf')
            doc2 = FSInputFile(path=rf'./Reports/{now_date}/Сводка_{now_date}.pdf')
